

# DevTITANS 09 - HandsOn Final - Equipe 03

Bem-vindo ao repositório da Equipe 03 no DevTITANS! A equipe irá criar 2 joysticks com acelerômetro (que implementem o protocolo do SNES), integrá-los a uma Raspberry Pi terá o AOSP instalado e, através disso tudo, controlar 2 braços robóticos ao mesmo tempo.

- [Contribuidores](#contribuidores)
- [Recursos](#recursos)
- [Uso](#uso)
- [Contato](#contato)

## Contribuidores

<img src="https://github.com/DevTITANS05/Hands-On-Linux-fork-/assets/21023906/85e61f3e-476c-47a4-82d5-4054e856c67b" width="180" >
<img src="https://github.com/DevTITANS05/Hands-On-Linux-fork-/assets/21023906/85e61f3e-476c-47a4-82d5-4054e856c67b" width="180" >
<img src="https://github.com/DevTITANS05/Hands-On-Linux-fork-/assets/21023906/85e61f3e-476c-47a4-82d5-4054e856c67b" width="180" >
<img src="https://github.com/DevTITANS05/Hands-On-Linux-fork-/assets/21023906/85e61f3e-476c-47a4-82d5-4054e856c67b" width="180" >
<img src="https://github.com/DevTITANS05/Hands-On-Linux-fork-/assets/21023906/85e61f3e-476c-47a4-82d5-4054e856c67b" width="180" >
<img src="https://github.com/DevTITANS05/Hands-On-Linux-fork-/assets/21023906/85e61f3e-476c-47a4-82d5-4054e856c67b" width="180" >

- **Thiago Pereira Lopes Chaves:** Criar o repositório do trabalho. Trabalhar na instalação/configuração do AOSP na Raspberry Pi.
- **Laura Nunes Belém:** Confeccionar os 2 joysticks e implementar os seus firmwares. Confeccionar os braço robóticos.
- **Raphael Vasconcelos Nunes de Mello:** Implementar o driver para comunicação com os joystick na Raspberry Pi.
- **William Roberto dos Santos Pereira:** Trabalhar na comunicação entre os 2 Joysticks X Raspberry Pi X 2 Braços Robóticos.
- **Caio Cesar Faneco Gonzaga:** Trabalhar na comunicação entre os 2 Joysticks X Raspberry Pi X 2 Braços Robóticos.

## Recursos

{Liste os recursos necessários como sensores, dispositivos, etc.}
1. Raspberry Pi
  a. Modelo: Mín. Raspberry Pi 4 Model B / Máx. Raspberry Pi 5 Model B.
  b. Memória: Mín. 4 GB RAM / Máx. 8 GB RAM.


## Uso

{Instruções para reproduzir as alterações no AOSP}
    
## Contato

Para perguntas, sugestões ou feedback, entre em contato com o mantenedor do projeto em [maintainer@example.com](mailto:maintainer@example.com).
